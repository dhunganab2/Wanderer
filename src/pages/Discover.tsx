    ...
    const loadDiscoveryUsers = async () => {
      ...
      try {
        ...
        if (firebaseUsers.length > 0) {
          ...
          // Fallback to sample users if no Firebase users available
          const fallbackUsers = sampleUsers.filter(user => 
            user.id !== authUser.uid && !swipedUserIds.includes(user.id)
          );
          setDiscoveryUsers(fallbackUsers);
        }
      } catch (error) {
        console.error('Error loading discovery users:', error);
        // Fallback to sample users on error
        const fallbackUsers = sampleUsers.filter(user => 
