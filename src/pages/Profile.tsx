...
export default function Profile() {
  const { user, updateUserProfile, loading, error } = useUserProfile();
  const [isEditing, setIsEditing] = useState(false);
  const [editForm, setEditForm] = useState(user || sampleUsers[0]);
  ...
  const displayUser = user || sampleUsers[0];

  const handleSave = async () => {
    if (!user) {
      toast.error('Cannot save profile: not authenticated');
      return;
    }
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-4 text-muted-foreground">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              Member since {new Date(displayUser.joinDate).getFullYear()}
                            </div>
                          </div>
                        </>
                      )}
                      ...
                      {!isEditing ? (
                        ...
                        <>
                          ...
                          <Button
                            ...
                            onClick={handleSave}
                            className="px-6"
                            disabled={isUploading || isSaving || loading}
                          >
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  {isEditing ? (
                    ...
                    <p className="text-foreground leading-relaxed">{displayUser.bio}</p>
                  )}
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {displayUser.travelStyle.map((style) => {
                          const styleOption = travelStyleOptions.find(s => s.id === style);
                          return (
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-lg font-semibold text-foreground">
                          {displayUser.nextDestination}
                        </h3>
                        <p className="text-muted-foreground">{displayUser.travelDates}</p>
                      </div>
                      <Button 
                        variant="outline" 
                        size="sm"
